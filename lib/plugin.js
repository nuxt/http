...
class HTTP {
  ...
  setHeader(name, value, scopes = 'common') {
    for (let scope of Array.isArray(scopes) ? scopes : [ scopes ]) {
      if (!value) {
        delete this._defaults.headers[scope][name];
        return
      }
      this._defaults.headers[scope][name] = value
    }
  }
...
export default (ctx, inject) => {
  ...
  if (process.server && ctx.req && ctx.req.headers) {
    ...
    defaults.headers.common = { ...reqHeaders, ...defaults.headers.common }
  }
  <% } %>

  if (process.server) {
    // Don't accept brotli encoding because Node can't parse it
    defaults.headers.common['accept-encoding'] = 'gzip, deflate'
  }
